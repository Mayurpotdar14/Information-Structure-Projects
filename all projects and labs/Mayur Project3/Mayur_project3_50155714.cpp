...
int main(void)
{
    ...
    srand(NULL);
            ...
            cout<<"INPUT:- ";
            s.generateRandomNumbers(array_data,nsize);// generate new random numbers
            s.insertion_Sort(array_data,nsize);// sort data using selection sorting algorithm
            s.displayData(array_data,nsize);
            end_val = clock();
            cout<<"\nSelection sort time :- [ "<<double(start_val-end_val)/CLOCKS_PER_SEC<<" ] Sec\n";
            ...
            if(nval1 > nval2 && nval1 > nval3)
                cout<<"\nInsertion sort takes less time :-  "<< nval1;
            else if(nval2 > nval1 && nval2 > nval3)
                cout<<"\Selection sort takes less time :-  "<< nval2;
            else if(nval3 > nval1 && nval3 > nval2)
