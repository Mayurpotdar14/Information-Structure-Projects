...
void Sorting_tech::generateRandomNumbers(int data[],int nsize)
{
...
int Sorting_tech::displayData(int data[], int nSize)
{
    for(int i=0;i<nsize;i++)
    {
        cout<<data[i]<<" ";
...
int main(void)
{
    ...
    srand(NULL);
            ...
            s.selectionSort(data,nsize);//sort data using selection sorting algorithm
            ...
            cout<<"INPUT:- ";
            s.generateRandomNumbers(data,nsize);// generate new random numbers
            s.insertionSort(data,nsize);// sort data using insertion sorting algorithm
            s.displayData(data,nsize);
